...
const roleColors = {
  ...
  'Team Leader': 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20',
...
function PersonCard({ person, size = 'normal' }) {
  ...
  return (
    <div className="group bg-surface-card rounded-2xl p-6 border border-border shadow-card card-hover text-center h-full flex flex-col items-center">
      <div
        className={`${avatarSize} rounded-full overflow-hidden bg-gradient-to-br ${person.color} flex items-center justify-center text-white font-display font-bold shadow-lg ring-4 ring-surface-card group-hover:scale-110 group-hover:rotate-3 transition-all duration-500`}
      >
        {person.image ? (
          <img
            src={person.image}
  ...
  {person.linkedin && (
    <a
      ...
      className="w-8 h-8 rounded-lg bg-surface-alt flex items-center justify-center text-text-muted hover:text-primary hover:bg-primary-50 dark:hover:bg-primary-50/10 transition-all duration-300"
